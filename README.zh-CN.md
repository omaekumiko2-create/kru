...
> **先连接一次，再保存一次，然后像平常一样描述任务：Agent 负责操作流程，KRU 只在最后一步于本地使用凭据。**

只需告诉 Agent **KRU 项目名称 + 想完成的事情**，无需说明密码、Token 或私钥的值，也不需要记忆 MCP 工具名。

| 场景 | 可以这样说 |
| --- | --- |
| **连接 VPS** | `使用 KRU 中的「Production VPS」连接服务器，检查磁盘、内存和 Docker 服务状态。` |
| **调用 API** | `使用 KRU 中的「Cloudflare API」凭据，列出 example.com 当前的 DNS 记录。` |
| **网页登录** | `在当前登录页，使用 KRU 中的「Work Account」依次填写账号、密码和 TOTP，然后继续登录。` |
| **CLI 认证** | `启动 gh 登录流程，并使用 KRU 中的「GitHub Token」完成认证。` |

引号内填写的是你在 KRU 中保存的项目名称。项目名称越清晰，Agent 就越容易选中正确的凭据。
