...
直接告诉 Agent 要使用哪个 KRU 项目、完成什么任务；也可以只要求它在遇到登录或认证时使用 KRU。无需在对话中粘贴密码。

| 用法 | 你可以直接这样说 |
| --- | --- |
| **项目名称 + 任务** | `使用 KRU 中的「TraceScale GPU VPS」，检查 GPU、磁盘和 Docker 状态。` |
| **需要登录时使用** | `继续完成这个任务；遇到需要登录的步骤时，使用 KRU。` |
| **连接服务器** | `连接 KRU 中的「Production VPS」，部署当前项目并检查运行状态。` |
| **调用 API** | `使用 KRU 中的「Cloudflare API」，列出当前账号下的域名。` |
| **网页登录** | `打开 Cloudflare 控制台；登录时使用 KRU 中的「Cloudflare Account」。` |

项目名称就是你保存在 KRU 中的名称。写明项目名称最准确；不确定时，也可以让 Agent 从 KRU 提供的项目中选择。
