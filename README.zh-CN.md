...
不需要记忆命令，也不用在对话中粘贴凭据。直接把 KRU 项目名称和任务一起告诉 Agent：

| 用法 | 你可以直接这样说 |
| --- | --- |
| **项目名称 + 任务** | `使用 KRU 项目「TraceScale GPU VPS」，检查 GPU、磁盘和 Docker 状态。` |
| **需要认证时使用** | `继续完成部署；如果中途需要登录或认证，使用 KRU。` |
| **连接服务器** | `使用 KRU 项目「Production VPS」连接服务器，部署当前项目并检查服务状态。` |
| **调用 API** | `使用 KRU 项目「Cloudflare API」，列出当前账号下的域名。` |
| **网页登录** | `打开 Cloudflare 控制台；需要填写账号、密码或验证码时，使用 KRU 项目「Cloudflare Account」。` |

项目名称就是保存在 KRU 中的名称。写明名称最准确；不确定时，也可以让 Agent 从 KRU 提供的项目中选择。
...
本地 `stdio` MCP 只在 Agent 调用时启动，无需常驻远程服务，日常使用几乎无感。
...
六位 PIN 用于锁定 GUI 中的明文查看。它是用户查看锁，不是保险库加密密钥。当前版本不提供 PIN 找回或重置流程。
