...
KRU 负责两者之间的最后一步。凭据只需保存一次；连接 Agent 后，登录或认证时由 Agent 把最终操作交给 KRU。KRU 在本地解密并使用凭据，隐藏值无需进入对话或模型上下文。
...
2. **保存一个项目**  
   只添加需要的模块：账号、密码、API 凭据、私钥、TOTP、主机、端口、URL 或自定义字段。

3. **连接 Agent**  
   打开 **设置 → Agent 接入**，注册已支持的客户端，然后新建一个 Agent 会话。
