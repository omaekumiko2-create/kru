...
连接 KRU 后，不需要学习新的命令格式，也不要把密码写进对话。像平常一样告诉 Agent 要做什么：

| 表达方式 | 示例 |
| --- | --- |
| **项目名称 + 任务** | `使用 KRU 中的「项目名称」，完成「要完成的任务」。` |
| **需要登录时使用** | `继续完成这个任务；如果过程中需要登录，使用 KRU。` |
| **需要认证时使用** | `调用这个 API；如果需要认证，使用 KRU 中合适的项目。` |
| **连接 VPS** | `使用 KRU 中的「Production VPS」，检查服务器的磁盘、内存和 Docker 状态。` |
| **网页登录** | `打开 Cloudflare 控制台；需要登录时使用 KRU 中的「Cloudflare Account」。` |

「项目名称」就是保存在 KRU 中的名称。名称清晰时，Agent 能更准确地选择对应项目；未指定名称时，Agent 也可以从 KRU 提供的项目中选择。
